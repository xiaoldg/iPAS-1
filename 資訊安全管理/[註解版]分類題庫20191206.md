...
```
```
預防性控制(Preventive)：https://wiki.mbalib.com/zh-tw/%E9%A2%84%E9%98%B2%E6%80%A7%E6%8E%A7%E5%88%B6
是為了避免產生錯誤或儘量減少今後的更正性活動，是為了防止資金、時間或其他資源的損耗而採取的一種預防保證措施。
檢測性控制(Detective)：https://wiki.mbalib.com/zh-tw/%E6%A3%80%E6%9F%A5%E6%80%A7%E6%8E%A7%E5%88%B6
是指將己經發生或存在的錯誤檢查出來的控制。
指令性控制(Directive)：
為了實現有利結果而進行的控制。
糾正性控制(Corrective)：https://wiki.mbalib.com/wiki/%E6%9B%B4%E6%AD%A3%E6%80%A7%E6%8E%A7%E5%88%B6
是指為了發現工作中存在的問題以便進行更正而進行的控制。
```
### 2.1.2.資訊資產分類及分級
```
...
雲端服務（Cloud Service）：
是一項結合雲端運算（Cloud Computing）、雲端儲存（CloudStorage）、網路連線與管理的新時代網際網路服務。

雲端運算（英語：cloud computing）：
是一種基於網際網路的運算方式，通過這種方式，
共享的軟硬體資源和資訊可以按需求提供給電腦各種終端和其他裝置，使用服務商提供的電腦基建作運算和資源。

服務模式：
軟體即服務（SaaS）
  消費者使用應用程式，但並不掌控作業系統、硬體或運作的網路基礎架構。
  例如：Adobe Creative Cloud，Microsoft CRM與Salesforce.com
平台即服務（PaaS）
  消費者使用主機操作應用程式。
  例如：Google App Engine
基礎設施即服務（IaaS）
  消費者使用「基礎運算資源」，如處理能力、儲存空間、網路元件或中介軟體。
...
```
```
https://zh.wikipedia.org/zh-tw/%E9%96%89%E8%B7%AF%E9%9B%BB%E8%A6%96

閉路電視（英語：Closed-Circuit Television，簡稱CCTV）：
是指在特定的區域進行視訊傳輸，並只在固定迴路裝置裡播放的電視系統。
```
...
```
```
https://www.e-ways.com.tw/index.php?option=articles&task=showpage&id=62

企業資源規劃系統（Enterprise Resource Planning），簡稱ERP系統，
是一個以會計為導向的資訊系統，利用模組化的方式，用來接收、製造、運送和結算客戶訂單所需的整個企業資源，
將原本企業功能導向的組織部門轉化為流程導向的作業整合，進而將企業營運的資料，轉化為使經營決策能更加明快，
並依據強調資料一致性、即時性及整體性的有效資訊。
```
```
...
```
```
https://dotblogs.com.tw/dotjason/2009/04/23/8154

資訊安全R&R粗略的分為：
Executive Management(執行經理)、Information System Security professional(資訊系統安全專家)、
Owners(擁有者)、Custodians(托管者)、Auditor(稽核者)、User(使用者)、IS/IT Function。

Executive management：
  必須要能承諾並推動資訊安全的政策、目標、並且承擔所有保護資產的責任，並且要能同意接受風險的權衡。
ISSP：
  有責任去設計、規劃、導入、管理及覆核企業的Security Policy及SPGB…等等。

Owners則為別擁有Data(資料)、System(系統)的關鍵角色，
Data Owners：
  必須負擔起Information Classification(資訊分類)、並分配資訊予Custodians，
  並且依循著Security police來執行安全作業，
...
原文網址：https://kknews.cc/career/33kqabg.html
```
...
```
```
https://wiki.mbalib.com/zh-tw/%E5%AE%9A%E9%87%8F%E9%A3%8E%E9%99%A9%E5%88%86%E6%9E%90
https://wiki.mbalib.com/zh-tw/%E9%A3%8E%E9%99%A9%E8%AF%84%E4%BC%B0
定量風險分析：
  對通過定性風險分析排出優先順序的風險進行量化分析。

年度發生率(Annualized Rate of Occurrence，ARO)—— 即威脅在一年內估計會發生的頻率。
暴露因數(Exposure Factor，EF)—— 特定威脅對特定資產造成損失的百分比，或者說損失的程度。
均線（Moving Average， MA）：
  移動平均（英語：moving average，MA），又稱「移動平均線」簡稱均線，是技術分析中一種分析時間序列數據的工具。
```
### 2.2.4.風險處理
...
```
```
https://wiki.mbalib.com/zh-tw/%E9%A3%8E%E9%99%A9%E6%8E%A5%E5%8F%97%E7%AD%96%E7%95%A5
[風險接受策略]是一種由企業自己承擔風險事故所致損失的一種財務風險管理技術。

https://wiki.mbalib.com/zh-tw/%E9%A3%8E%E9%99%A9%E8%BD%AC%E7%A7%BB
[風險轉移]是指將風險及其可能造成的損失全部或部分轉移給他人。例如：保險

https://wiki.mbalib.com/zh-tw/%E9%A3%8E%E9%99%A9%E9%81%BF%E5%85%8D
[風險避免]即放棄或不進行可能帶來損失的活動或工作。例如，為了避免洪水風險，可以把工廠建在地勢較高、排水方便的地方
```
```
...
存取控制目錄（Access Control List ， ACL）：
  是使用以存取控制矩陣為基礎的存取控制表，每一個（檔案系統內的）物件對應一個串列主體。
  存取控制串列由存取控制條目（access control entries，ACE）組成。
...
```
```
https://wiki.mbalib.com/zh-tw/%E9%A2%84%E9%98%B2%E6%80%A7%E6%8E%A7%E5%88%B6

https://wiki.mbalib.com/zh-tw/%E9%A2%84%E9%98%B2%E6%80%A7%E6%8E%A7%E5%88%B6
預防性控制是為了避免產生錯誤或儘量減少今後的更正性活動，
是為了防止資金、時間或其他資源的損耗而採取的一種預防保證措施。

https://wiki.mbalib.com/zh-tw/%E6%A3%80%E6%9F%A5%E6%80%A7%E6%8E%A7%E5%88%B6
檢查性控制是指將己經發生或存在的錯誤檢查出來的控制。

https://wiki.mbalib.com/zh-tw/%E6%8C%87%E5%AF%BC%E6%80%A7%E6%8E%A7%E5%88%B6
指令性控制(Directive)：
  為了實現有利結果而進行的控制。
  
https://wiki.mbalib.com/zh-tw/%E7%BA%A0%E6%AD%A3%E6%80%A7%E6%8E%A7%E5%88%B6
糾正性控制(Corrective)：
  為了發現工作中存在的問題以便進行更正而進行的控制。
```
...
```
```
https://www.netadmin.com.tw/netadmin/zh-tw/viewpoint/4A0B43F664934436929B828D539CA3D5
職務區隔（Segregation of Duties）：
  這項控制措施的目的是，針對工作職務和責任應予以清楚區隔，
  降低資訊受到未經授權的修改，以及資訊資產受到誤用或濫用的情況發生。
  
https://zh.wikipedia.org/zh-tw/%E6%9C%80%E5%B0%8F%E6%9D%83%E9%99%90%E5%8E%9F%E5%88%99
最小權限原則（Principle of Least Privilege）：
  最小權限原則是要求計算環境中的特定抽象層的每個模組如進程、用戶或者電腦程式只能訪問當下所必需的信息或者資源。
  賦予每一個合法動作最小的權限，就是為了保護數據以及功能避免受到錯誤或者惡意行為的破壞。

https://dotblogs.com.tw/dotjason/2009/04/29/8240
必要知道原則（Need-to-know Principle）：
  完成一件任務所需要知道的內容及所需要知道的流程，僅給予完成工作的最少量的應知事項即可。
  
https://zh.wikipedia.org/zh-tw/%E4%BB%A5%E8%A7%92%E8%89%B2%E7%82%BA%E5%9F%BA%E7%A4%8E%E7%9A%84%E5%AD%98%E5%8F%96%E6%8E%A7%E5%88%B6
...
雙重認證（英語：Two-factor authentication，縮寫為2FA），
又譯為雙重驗證、雙因子認證、雙因素認證、二元認證，
又稱兩步驟驗證（2-Step Verification，又譯兩步驗證），
是一種認證方法，使用兩種不同的元素，合併在一起，來確認使用者的身分，是多因素驗證中的一個特例。
...
```
[時間基礎的一次密碼（Time-based One-Time Password， TOTP）](https://zh.wikipedia.org/wiki/%E5%9F%BA%E4%BA%8E%E6%97%B6%E9%97%B4%E7%9A%84%E4%B8%80%E6%AC%A1%E6%80%A7%E5%AF%86%E7%A0%81%E7%AE%97%E6%B3%95)
[多因子認證法（Multifactor）](https://zh.wikipedia.org/wiki/%E5%A4%9A%E9%87%8D%E8%A6%81%E7%B4%A0%E9%A9%97%E8%AD%89)
[相互認證法（Mutual Authentication）](https://en.wikipedia.org/wiki/Mutual_authentication)
```
時間基礎的一次密碼（Time-based One-Time Password， TOTP）：
  基於時間的一次性密碼演算法（TOTP）是一種根據預共享的金鑰與目前時間計算一次性密碼的演算法。
  弱點和缺陷：
    儘管攻擊者需要即時代管憑證，而不能之後收集，但是TOTP代號跟密碼一樣可能被釣魚。
    不限制登入嘗試的TOTP實現容易被暴力破解，因此嘗試次數限制必須非常少。

多因子認證法（Multifactor）：
  多重要素驗證（英語：Multi-factor authentication，縮寫為 MFA），又譯多因子認證、多因素驗證、多因素認證，
  是一種電腦存取控制的方法，用戶要通過兩種以上的認證機制之後，才能得到授權，使用電腦資源
  
相互認證法（Mutual Authentication）：
  Mutual authentication or two-way authentication refers to two parties authenticating each other at the same time, 
...
```
[雜湊注入(Pass-the-Hash)](https://blogs.technet.microsoft.com/technet_taiwan/2016/03/29/pass-the-hash-pth-ptt-pass-the-ticket-2/)
```
...
```
[傳遞票證（Pass the Ticket）](https://attack.mitre.org/techniques/T1097/)
[傳遞票證（Pass the Ticket）](https://www.tcrc.edu.tw/new/new-list/pass-the-ticket)
[傳遞票證（Pass the Ticket）](https://www.netadmin.com.tw/netadmin/zh-tw/technology/1D41DA77449147298AF1902AD8495521)
[雜湊注入(Pass-the-Hash)](https://blogs.technet.microsoft.com/technet_taiwan/2016/03/29/pass-the-hash-pth-ptt-pass-the-ticket-2/)
```
...
一次性密碼（英語：one-time password，簡稱OTP），又稱動態密碼或單次有效密碼，
是指計算機系統或其他數位裝置上只能使用一次的密碼，有效期為只有一次登入會話或交易。
```
```
...
```
```
生物辨識技術（biometrics，也稱生物測定學），是指用數理統計方法對生物進行分析，
現在多指對生物體（一般特指人）本身的生物特徵來區分生物體個體的電腦技術。
...
LDAP 是一種目綠服務，可使用 LDAP 記錄各種的人員資訊
```
```
...
```
```
[這題很重要][77]下列何種生物辨識方式之交叉錯誤率（Crossover Error Rate， CER）最低？
(A) 語音辨識   (B) 掌形辨識   (C) 手寫辨識  (D) 虹膜辨識
```
```
[這題很重要][這題很重要][這題很重要][這題很重要][這題很重要][這題很重要]
https://dotblogs.com.tw/dotjason/2009/07/31/9764

FRR(False Reject Rate):錯誤拒絕率，把對的驗證為錯誤的屬於Type I error
FAR(FalseAccept Rate):錯誤接受率，把錯誤的驗證為對的屬於Type II error
CER(Crossover Error Rate):交叉錯誤率，集合FRR及FAR兩個曲線的交叉點。
[這題很重要][這題很重要][這題很重要][這題很重要][這題很重要][這題很重要]
```
```
...
單一登入（英語：Single sign-on，縮寫為 SSO），又譯為單一簽入，
一種對於許多相互關連，但是又是各自獨立的軟體系統，提供存取控制的屬性。
```
```
...
```
```
https://moica.nat.gov.tw/what.html
自然人憑證：
  可以在網路上作資料交換時，如同網路身分證辨識雙方身分。
...
```
```
https://zh.wikipedia.org/wiki/%E4%BB%A3%E7%A0%81%E5%AE%A1%E6%9F%A5
原始碼測試(Source Code Review)：
  指對電腦原始碼系統化地審查，常用軟體同行評審的方式進行，
  其目的是在找出及修正在軟體開發初期未發現的錯誤，提升軟體品質及開發者的技術。
  
https://zh.wikipedia.org/wiki/%E5%A3%93%E5%8A%9B%E6%B8%AC%E8%A9%A6_(%E8%BB%9F%E9%AB%94)
壓力測試(Stress Testing)：
  壓力測試對於關鍵任務軟體格外的重要，但可以適用於各種的軟體。
  壓力測試一般較強調軟體在高負載下的健壯性、可用性及異常處理，以及哪些在一般使用環境下算是正常行為。
...
滲透測試(Penetration Test)：
  滲透測試是指一個具備資安知識與經驗、技術人員受僱主所託，為僱主的網路裝置、主機，
  類比駭客的手法對網路或主機進行攻擊測試，為的是發掘系統漏洞、並提出改善方法，通常是出於善意。
```
```
